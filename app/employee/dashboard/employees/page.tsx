...
import { UserPlus, Trash2, Edit, X, Eye, IndianRupee, Building2, Mail, Phone, LayoutDashboard } from 'lucide-react';
...
export default function EmployeesDashboard() {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-5">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <LayoutDashboard className="w-6 h-6 text-slate-800" /> Staff Management Directory
          </h1>
          <p className="text-sm text-slate-500">Onboard, track, modify, or view employee profile configurations</p>
        </div>
        {isAdmin && (
          <button
      ...
      {/* Grid Table */}
      {loading ? (
        <p className="text-slate-400 text-sm py-6">Loading workforce data...</p>
      ) : employees.length === 0 ? (
        <p className="text-slate-400 text-sm bg-slate-50 p-6 text-center rounded-2xl">No employees onboarded.</p>
      ) : (
                  ...
                  <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-slate-900 flex items-center gap-1.5">
                            ...
                            {emp.emp_code && <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">#{emp.emp_code}</span>}
                          </p>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Link href={`/dashboard/employees/${emp.id}`} className="p-1.5 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition">
                        ...
                        {isAdmin && (
                          <>
                            ...
                            <button onClick={() => handleDeleteEmployee(emp.id, emp.name || 'this employee')} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition">
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full space-y-5 shadow-2xl border border-slate-100">
            ...
            <form onSubmit={handleUpdateEmployee} className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Base Monthly Salary (₹)</label>
