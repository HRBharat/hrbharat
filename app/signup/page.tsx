...
export default function SmartRegistrationGate() {
  ...
  // NEW: Geofencing Location States
  ...
  const handleRegisterPipeline = async (e: React.FormEvent) => {
    ...
    if (authData?.user) {
      ...
      if (preRegisteredEmployee) {
        ...
        // Atomic swap: Update the employee row placeholder ID to match their live User Auth ID
        await supabase
          .from('employees')
        ...
        // NO MATCH: The user is registering a brand new company
        const freshCompanyId = crypto.randomUUID();
        finalCompanyId = freshCompanyId;

        // Initialize their corporate entity workspace with custom GPS Geo-fence parameters
        await supabase
          .from('companies')
          .insert({
            id: freshCompanyId,
      ...
      // 3. Create their central authorization profile node
      await supabase
        .from('profiles')
        .insert({
          id: authData.user.id,
            ...
            <div className="border-t border-slate-100 pt-4 space-y-3">
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={handleDetectCurrentLocation} disabled={fetchingGeo} className="text-[10px] font-bold text-teal-700 hover:text-teal-800 flex items-center space-x-1 bg-white px-2 py-1 rounded-lg border border-slate-200 shadow-xs">
              ...
              <p className="text-[9px] text-slate-400 font-medium">Employees: You can ignore the company/location boxes. Your account auto-links to your organization profile based on your work email address.</p>
            </div>

            <button type="submit" disabled={submitting} className="w-full bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold py-2.5 px-4 rounded-xl transition-all shadow-sm mt-2">
